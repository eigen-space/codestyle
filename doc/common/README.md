...
e. После корректировки замечаний и реализации предложений должен запрашиваться
...
как для просматривающего изменения, так и для автора изменений. 
...
1. Создаётся предложение в канал `#proposals`. 
2. Обсуждается и голосуется в течение итерации.
3. Если предложение принято, оно заносится в документацию по стилю кода. 
   По возможности оно автоматизируется (правила для tslint / конфигурация для IDEA). 
   На это создаётся отдельная задача.
   
TODO: Вносить предложения через pull request'ы в текущий репозиторий

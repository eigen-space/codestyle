...
b. Макконнелл С. Совершенный код. Мастер#класс / Пер. с англ. — М. : Издательство «Русская редакция», 2010. — 896 стр. : ил.
...
g. Intellij IDEA. File | Settings | Editor | Code Style | JavaScript
...
#### 2.1.1. Скобки для управляющих конструкций
...
a. Пустой блок допустимо использовать только для цикла `while`. При этом закрывающая 
   скобка должна располагаться на следующей строке. Для других управляющих конструкций 
   пустой блок использовать запрещено.

```javascript
    // Плохо
    while (waitSignalFromBeaconService()) {}
...
b. Не должно быть пустых методов и функций. В этом случае они, вероятно, 
   не используются и должны быть удалены в соответствии с п. 8.3. Исключение 
   составляет использование пустых методов при объявлении интерфейса через 
   класс с пустыми методами, т.к. нет родной конструкции языка для объявления интерфейса. 
   При этом открывающая и закрывающая фигурные скобки `{}` располагаются на одной строке 
   и следуют через пробел за сигнатурой метода.

```javascript
    // Плохо, т.к. просто нужно удалить бесполезную функцию
    ...
    class SomeInterface {
        ...
        methodHasToBeImplemented() {}

        anotherMethodHasToBeImplemented() {}
    }

    // Некоторый абстрактный класс
    class SomeAbsctractClass {
...
#### 2.3.1. В одной строке должно быть не более одного выражения
...
#### 2.3.2. Выражение обязательно завершается точкой с запятой (";")

Каждое выражение должно завершаться знаком точка с запятой ";". Не следует полагаться 
...
#### 2.4.1. В каком месте разрывать выражение
...
#### 2.4.2. Отступ при переносе части выражения
...
Управляющие конструкции не должны записываться в одну строку.
...
#### 2.4.6. Перенос длинных математических выражений
...
#### 2.4.8. Перенос полей объектных литералов

Поля объектного литерала переносятся, если:
 
1. Их количество более 3.
2. Поля в качестве значения имеют выражение, функцию или объект.
3. Длина неперенесённого объекта превышает установленный лимит строки.
...
#### 2.4.9. Перенос функций, переданных в качестве параметров
...
#### 2.4.10. Перенос цепочек методов

a. Выражение должно переноситься по точке.
...
b. Если в цепочке вызовов есть метод, параметры которого переносятся на несколько строк, то следует эту цепочку
...
#### 2.4.11. Перенос запятой при переносе выражений
...
#### 2.5.1. Пробельные места в вертикальном направлении
...
a. Перед конструктором и любым методом класса.
...
c. После закрывающей фигурной скобки `}`, если за ней не следует строка, 
...
a. Перед строкой с закрывающей скобкой, завершающей функцию, метод, конструктор, 
   объект, массив, управляющую конструкцию (`if-else`, `for`, `while` и т.д.). 

b. В начале функции (метода). Исключение составляет только добавление пустой строки 
   для вложенной функции, которые встречаются при написании тестов.

c. Между последовательно идущими объявлениями полей класса.
...
#### 2.5.2. Пробельные места в горизонтальном направлении
...
#### 2.5.3. Горизонтальное выравнивание
...
### 2.6. Использование скобок для группировки
...
a. Комментарий должен иметь тот же отступ, что и код, к которому он относится.
...
b. Текст комментария должен быть отделен от знака комментария одиночным пробелом.
...
#### 2.7.3. Не использовать комментарии в конце строки
...
#### 2.8.2. Группировка аксессоров (get/set)
...
#### 3.1.1. Использовать `const` и `let`
...
#### 3.1.2. Объявлять по одной переменной за один раз
...
#### 3.1.3. Объявлять ближе к месту использования
...
#### 3.1.4. Не следует объявлять лишние переменные
...
#### 3.1.5. Одна обязанность у переменной
...
#### 3.1.6. Объявление переменной с присваиванием ей значения по умолчанию
...
```typescript
    // Плохо
    allowedEntityTypes = [] as string[];
    someObject = {} as SomeInterface;
...
```typescript
    // Плохо
    stringVariable: string = 'some string value';
    numericVariable: number = 17;
    map: Map<EntityId, Entity> = new Map<EntityId, Entity>();
    map: Map<EntityId, Entity> = new Map();
    
    // Хорошо
    stringVariable = 'some string value';
    numericVariable = 17;
    map = new Map<EntityId, Entity>();
```

### 3.2.  Массивы

#### 3.2.1. Использование запятой для последнего элемента
...
#### 3.2.2. Избегать использования конструктора `Array`
...
В случае необходимости создать именно массив заданной длины допустимо использовать 
`new Array(length)`.

#### 3.2.3. Нечисловые свойства

Нельзя добавлять и использовать новые свойства для массива, кроме числовых 
(т.е. тех, что добавляются при добавлении элементов в массив) и `length`. 
Для этих целей использовать `Map` или `Object`.

#### 3.2.4. Оператор "spread"
...
#### 3.3.1. Использование запятой после последнего свойства
...
#### 3.3.2. Использование конструктора `Object`
...
#### 3.3.3. Кавычки для свойств объекта
...
#### 3.3.7. Использовать Map для динамически формируемых объектов
...
Перечисления объявляются с помощью объектных литералов. При этом используется 
ключевое слово `const`, название перечисления записывается в `UpperCamelCase`, 
в конце названия добавляется `Type`, а поля перечисления задаются в `UPPER_SNAKE_CASE`.
...
В качестве значения для поля объекта может использовать не только строка. 
Вполне допустимо использовать объекты, если это нужно.

```javascript
    const TripStatusType = {
        PLAN: new TripStatus({ name: 'plan', title: 'Запланирован', icon: 'wait' }),
        EXEC: new TripStatus({ name: 'exec', title: 'Исполняется', icon: 'in-progress' })
    } 
```
...
a. Не нужно объявлять конструктор, если он не используется.

b. Для подклассов обязательно вызывать `super()` в самом начале конструктора, 
   до выполнения каких-либо других операций.

c. Для интерфейсов не должно быть объявления конструктора.

d. Инициализация объекта должна производиться в конструкторе, а не в отдельном методе, 
...
e. В конструкторе не нужно использовать `return`.
...
f. В конструкторе всегда принимать объект, даже если на вход мы ожидаем только одно поле 
...
b. После создания объекта класса свойства не должны ни добавляться, ни удаляться 
...
d. Статические поля добавлять после объявления класса вместо добавления их в виде 
   getter/setter'ов.
...
#### 3.5.7. Аксессоры (`getter`-ы и `setter`-ы)
...
#### 3.5.8. Переопределение метода `toString`

При необходимости использовать текстовое представление экземпляра класса можно 
переопределить метод `toString` базового класса объекта.
...
```javascript
    ...
    // Выведется "test: Test { a: 1, b: 2 }" вместо "test: [object Object]"
    console.log('test: ' + test);
```

#### 3.5.9. Интерфейсы

На данный момент язык не поддерживает объявление интерфейсов. В связи с этим 
при необходимости интерфейс следует описывать с использование ключевого слова `class`, 
определяя в нём необходимый набор методов, который должен быть реализован другими 
классами.
...
#### 3.6.1. Использование классов вместо функций

Не должно быть отдельных функций. 

В устаревшем коде возможны случаи:

* С помощью функции и переопределения `prototype` этой функции объявлен класс 
  в старом стиле. В этом случае необходимо переписать функцию на класс.
* Отдельная функция. Должна быть включена в состав какого-то класса.

#### 3.6.2. Вложенные функции
...
#### 3.6.6. Spread-оператор

Использовать оператор `...` вместо `Function.prototype.apply`, когда требуется 
...
#### 3.7.1. Тип используемых кавычек
...
a. Всегда выбрасывать исключение, являющееся экземпляром класса ошибки приложения. 
   Никогда не использовать другие типы.

b. Не должно быть пустых блоков обработки исключения, т.е. "проглатывания" исключений.
...
c. Объявлять и использовать переменную, инициализация которой может не выполниться, 
...
#### 3.9.4. Оператор `switch-case`

a. Не следует использовать `switch` тогда, когда уместнее использовать `if-else if`. 
...
b. Каждый блок `case` должен завершаться оператором `break`, чтобы не было сквозного 
...
c. Не должно быть оператора `break` в секции `default`.
...
d. `switch` обязательно должен включать секцию `default`, даже если она при нормальной 
   логике работы не должна быть задействована. В этом случае в ней должно выбрасываться 
   исключение, поясняющее проблему.
...
```javascript
    ...
    switch (someValue) {
        ...
        case 'third':
            ...
            break;
        default:
            throw new AppError(`Unexpected value of texture: ${someValue}`);
    }
```

#### 3.9.5. Условия

a. Для сравнения двух операндов нужно использовать строгое равенство `===` (`!==`),  
...
b. Условия должны располагаться в направлении числовой оси. Как одиночные неравенства, 
...
c. При использовании знака равенства при сравнении переменной и литерала (или константы) 
...
d. Не следует разбивать условия на отдельные без необходимости, когда можно записать их 
через `||` или `&&`.
...
e. Не добавлять лишних группировок и скобок в условиях, если можно этого не делать.
...
f. По возможности избегать лишней вложенности.
...
g. По возможности избегать лишних ветвей `else`. Это уместно, когда одно из значений 
...
h. Не использовать тернарный оператор тогда, когда можно обойтись логическим оператором.
...
i. Недопустимо использовать вложенные тернарные операторы. В данном случае необходимо 
...
Допустимо использовать `this` только внутри конструктора класса и его методов. 
Не должно быть использования `this` для обращения к глобальным объектам, объектам 
контекста `eval` и каких-либо других случаев.

### 3.11. Преобразование примитивов

#### 3.11.1. Использование функций-обёрток
...
#### 3.11.2. Преобразование к типу Boolean
...
#### 3.11.3. Использование функций-обёрток над примитивами вместо функций `parseInt`
...
#### 3.11.4. Указывать систему счисления при использовании `parseInt`
...
#### 3.12.6. Модификация встроенных объектов и их прототипов

Не следует модифицировать встроенные объекты и их прототипы 
(`Object`, `Array`,  `Array.prototype` и т.д.), добавляя в них дополнительные 
свойства и методы.

### 3.13. Магические строки и числа
...
#### 3.13.1. Использовать константы вместо магических строк и чисел
...
#### 3.13.2. Допустимые варианты использования литералов
...
#### 3.14.1. Использовать `async/await`
...
#### 3.14.2. Не допускать вложенности `Promise`
...
#### 3.14.3. Использовать наиболее лаконичную запись
...
```javascript
    ...
    void sortStableInRange(begin, end) {
        // ...
    }
```

#### 3.17.2. Псевдокод и комментарии
...
#### 3.18.1. Создание Map на основе массива
...
#### 3.19.1. Указание задержки срабатывания
...
a. Имена классов должны соответствовать `UpperCamelCase`. Это касается как экспортируемых, 
...
b. Имя должно состоять из существительных и прилагательных.
...
c. Должно использоваться единственное число.
...
Имена методов должны соответствовать `lowerCamelCase`. Имена приватных методов должны 
начинаться с символа подчёркивания `_`.
...
#### 4.2.3. Имена перечислений

Имя перечисления должно соответствовать `UpperCamelCase`, как и для классов. 
В большинстве случаев уместно дополнить перечисление словом `Type` на конце, 
что будет дополнительно показывать, что это перечисление.
...
#### 4.2.4. Имена констант

a. Имена констант должны соответствовать `UPPER_SNAKE_CASE`: все буквы в верхнем регистре, 
...
b. Константа, объявленная внутри функции/метода, должна именоваться также, как переменная.
...
#### 4.2.5. Имена полей класса

Имена изменяемых полей класса (статичных или принадлежащих экземпляру) должны 
соответствовать `lowerCamelCase`. Имена приватных полей должны начинаться 
с нижнего подчёркивания `_`.
...
```javascript
    ...
    export default class ZoneService {
        ...
        constructor(zoneDataService) {
            // Добавляется _, т.к. приватные поля
            this._zoneDataService = zoneDataService;
            this._zonePromise = null;
            // Нет _, т.к. публичные поля
            this.configuration = new Configuration();
            this.fitler = new Filter();
...
#### 4.2.7. Имена локальных переменных
...
Константа, объявленная внутри функции, должна именоваться также, как переменная.

Имена переменных должны состоять из прилагательных и существительных.

#### 4.2.8. Имена параметров в setter'ах
...
### 4.3. Названия в верхнем регистре
...
### 4.5. Поля логического типа

a. Поля, которые содержат признак того, имеется ли какое-то свойство или нет (поле-флаг), обозначаются 
как `has<существительное>`. Например, `hasBorder`.

b. Поля, которые определяют возможность использовать какой-то функционал, обозначаются как `is<возможность>Enabled`.
Например, `isLoggingEnabled`.

c. Любой однозначный логический тип определяется причастием или отглагольным прилагательным. Например: 
`hidden`, `busy`, `disabled` и т.д.

### 4.6. Спецификации

#### 4.6.1. Именование переменных под объекты заглушек

Имя переменной-конструктора для заглушки должно быть в виде `Mock<имя класса, на экземляр которого делается заглушка>`.
Переменной, являющейся экземпляром класса-заглушки, необходимо давать имя в следующем формате: 
`mock<имя класса, на экземляр которого делается заглушка>`. 
...
TSDoc/JSDoc должен использоваться для публичного интерфейса классов и модулей, описывая назначение, входные, 
выходные параметры, исключения и возвращаемое значение.
...
b. Длина каждой строки JSDoc не должна превышать установленное ограничение на длину строки.

c. Комментарий пишется в Markdown. Например, ненумерованный список задаётся следующим образом:
...
d. Перенос строк для секции параметров выполняется с дополнительным отступом.
...
### 6.1. Покрытие кода спецификациями

a. При внесении изменений должны быть спецификации (тесты), которые описывают требования, 
которые эти изменения реализуют, и проверяются их. 

b. Когда пишем спецификации необходимо обращать внимание на процент покрытия кода 
спецификациями (code coverage). Необходимо доводить покрытие до 100%. Меньше 100% допускается только, 
если в классе есть методы, которые проверять не нужно (фиктивные, искусственные методы, которые
пробрасывают функциональность с одного уровня приложения на другой).
...
### 7.3. Неиспользуемые переменные, классы, методы

В коде не должно быть неиспользованных констант, методов, классов, переменных. 
Неиспользуемые выражения и конструкции нужно удалять.
...
b. Поправить правило переноса цепочек функций?

### 8.2. Принципы

a. Сервисы должны иметь конкретные методы, которые скрывают детали. Какие-то общие 
   методы должны быть приватными.
